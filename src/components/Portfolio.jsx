...
const Portfolio = () => {
  ...
  const handleSlide = (direction) => {
    if (swiperRef.current) {
      direction === 'next' ? swiperRef.current.swiper.slideNext() : swiperRef.current.swiper.slidePrev();
    }
  };
    ...
    <div className="relative py-16 md:py-20 lg:py-28 xl:py-[150px] overflow-hidden min-[1920px]:max-w-[1391px] min-[1920px]:mx-auto">
      <div className="container max-w-[1188px] mb-4 sm:mb-6 md:mb-8 lg:mb-12 flex items-end justify-between">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <button onClick={() => handleSlide('prev')} className="size-8 sm:size-10 rounded-full border border-black flex justify-center items-center xxl:absolute xxl:top-1/2 xxl:-translate-y-1/2 xxl:-left-[77px]"><Icons iconName="leftArrow" /></button>
          <button onClick={() => handleSlide('next')} className="size-8 sm:size-10 rounded-full border border-black rotate-180 flex justify-center items-center xxl:absolute xxl:top-1/2 xxl:-translate-y-1/2 xxl:-right-[77px]">
            <Icons iconName="leftArrow" /></button>
        </div>
      </div>

      <div className="!pl-6 overflow-visible">
        <Swiper ref={swiperRef} breakpoints={{ 320: { slidesPerView: 2 }, 708: { slidesPerView: 3 }, 1024: { slidesPerView: 4 }, 2540: { slidesPerView: 4 } }}
          loop spaceBetween={10} pagination={{ clickable: true, el: '.swiper-pagination' }} autoplay={{ delay: 3000, disableOnInteraction: false }}
